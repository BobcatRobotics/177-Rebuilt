...
import frc.robot.subsystems.vision.AprilTagVisionConstants.limelightConstants;
    ...
    DriverStation.isDSAttached() && getBotPoseMG2() != null) {
      ...
      if (swerve.getRotationRate().getZ() <= Units.degreesToRadians(720)) {
        if (getPoseValidMG2(swerve.getRotation())) {
          ...
          swerve.addVisionMeasurement(getBotPoseMG2(), getPoseTimestampMG2(),VecBuilder.fill(xyStdDev, xyStdDev, thetaStdDev));
        }
      }
    }
  }
