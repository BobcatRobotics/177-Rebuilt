...
public class IntakeSim implements IntakeIO {
    ...
    private TalonFXSimState flywheelMotorState;
    private TalonFXSimState positionMotorState; 
    private FlywheelSim m_motorPivotSimModel;
    private FlywheelSim m_motorVelocitySimModel;
    ...
    public IntakeSim() {
        ...
        m_motorPivotSimModel = new FlywheelSim(LinearSystemId.createFlywheelSystem(motorModel, .00003, 1), motorModel);
        m_motorVelocitySimModel = new FlywheelSim(LinearSystemId.createFlywheelSystem(motorModel, .00003, 1), motorModel);

        setUpLeftRollerMotor(leftRollerMotorGains);
        setUpRightRollerMotor(rightRollerMotorGains);
        setupPivotMotor(pivotMotorGains);

        positionMotorState = positionMotor.getSimState();
    ...
    public void simulationPeriodic() {
        ...
        flywheelMotorState = leftVelocityMotor.getSimState();
        double motorVoltage = flywheelMotorState.getMotorVoltage();
        // Feed Into Physics Simulation
        m_motorVelocitySimModel.setInputVoltage(motorVoltage);
        // Udpate SIM ( 20ms loop )
        m_motorVelocitySimModel.update(0.02);
        // get voltage ( rad/sec -> rotations/sec)
        double velocityRadPerSec = m_motorVelocitySimModel.getAngularVelocityRadPerSec();
        double velocityRotPerSec = velocityRadPerSec / (2 * Math.PI);
        // PUSH intop the TalonFX simulated Sensor the value
        flywheelMotorState.setRotorVelocity(-velocityRotPerSec);


        // Get Motor Output Voltage
        positionMotorState = leftVelocityMotor.getSimState();
        motorVoltage = flywheelMotorState.getMotorVoltage();
        // Feed Into Physics Simulation
        m_motorPivotSimModel.setInputVoltage(motorVoltage);
        // Udpate SIM ( 20ms loop )
        m_motorPivotSimModel.update(0.02);
        // get voltage ( rad/sec -> rotations/sec)
        velocityRadPerSec = m_motorPivotSimModel.getAngularVelocityRadPerSec();
