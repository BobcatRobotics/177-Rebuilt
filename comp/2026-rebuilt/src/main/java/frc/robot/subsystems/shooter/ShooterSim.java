...
public class ShooterSim implements ShooterIO {
    ...
    private TalonFXSimState shooterLeftMotorState;
    private FlywheelSim m_motorLeftSimModel;
    private TalonFXSimState shooterRightMotorState;
    private FlywheelSim m_motorRightSimModel;
    private TalonFXSimState shooterPositionMotorState;
    private FlywheelSim m_motorPositionSimModel;
    ...
    public ShooterSim() {
        ...
        m_motorRightSimModel = new FlywheelSim(LinearSystemId.createFlywheelSystem(motorModel, .00003, 1), motorModel);
        m_motorPositionSimModel = new FlywheelSim(LinearSystemId.createFlywheelSystem(motorModel, .00003, 1), motorModel);
        ...
        shooterLeftMotorState = dumperLeftUp.getSimState();
        shooterRightMotorState = dumperRightUp.getSimState();
        shooterPositionMotorState = adjustableHood.getSimState();
    ...
    public void lowTelemetry(ShooterIOInputs inputs) {
        ...
        inputs.statorCurrentOfAdjustableHoodPositionAmps = statorCurrentOfAdjustableHoodPositionAmps.getValue()
                .in(Amps);
    ...
    public void simulationPeriodic() {
        ...
        shooterLeftMotorState = dumperLeftDown.getSimState();
        double motorVoltage = shooterLeftMotorState.getMotorVoltage();
        // Feed Into Physics Simulation
        m_motorLeftSimModel.setInputVoltage(motorVoltage);
        // Udpate SIM ( 20ms loop )
        m_motorLeftSimModel.update(0.02);
        // get voltage ( rad/sec -> rotations/sec)
        double velocityRadPerSec = m_motorLeftSimModel.getAngularVelocityRadPerSec();
        double velocityRotPerSec = velocityRadPerSec / (2 * Math.PI);
        // PUSH intop the TalonFX simulated Sensor the value
        shooterLeftMotorState.setRotorVelocity(-velocityRotPerSec);

        // Get Motor Output Voltage
        shooterRightMotorState = dumperLeftDown.getSimState();
        motorVoltage = shooterRightMotorState.getMotorVoltage();
        // Feed Into Physics Simulation
        m_motorRightSimModel.setInputVoltage(motorVoltage);
        // Udpate SIM ( 20ms loop )
        m_motorRightSimModel.update(0.02);
        // get voltage ( rad/sec -> rotations/sec)
        velocityRadPerSec = m_motorRightSimModel.getAngularVelocityRadPerSec();
        velocityRotPerSec = velocityRadPerSec / (2 * Math.PI);
        // PUSH intop the TalonFX simulated Sensor the value
        shooterRightMotorState.setRotorVelocity(-velocityRotPerSec);

        // Get Motor Output Voltage
        shooterPositionMotorState = dumperLeftDown.getSimState();
        motorVoltage = shooterPositionMotorState.getMotorVoltage();
        // Feed Into Physics Simulation
        m_motorPositionSimModel.setInputVoltage(motorVoltage);
        // Udpate SIM ( 20ms loop )
        m_motorPositionSimModel.update(0.02);
        // get voltage ( rad/sec -> rotations/sec)
        velocityRadPerSec = m_motorPositionSimModel.getAngularVelocityRadPerSec();
        velocityRotPerSec = velocityRadPerSec / (2 * Math.PI);
        // PUSH intop the TalonFX simulated Sensor the value
        shooterPositionMotorState.setRotorVelocity(-velocityRotPerSec);
    }
